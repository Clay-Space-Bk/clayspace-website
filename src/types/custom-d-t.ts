...
/** Cart ids come from two sources: numeric ids on catalogue products, and
 *  synthetic string ids such as `ceramics-0` for items built from page data.
 *  The cart only ever compares ids for equality, never arithmetic. */
...
export interface CartItem {
  ...
  title: string;
  price: number;
  image: import("next/image").StaticImageData | string;
  link?: string;
  quantity: number;
}
...
export interface AppContextType {
  ...
  incQty: (id: CartItemId) => void;
  decQty: (id: CartItemId) => void;
  clearCart: () => void;
  cartCount: number;
  cartTotal: number;
}
